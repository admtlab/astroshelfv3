<?php
	...
	include "../db/general_sdss_query.php";
	...
	try{
		switch($command){
			...
			case "constructFromResults":
				...
				file_put_contents("/var/www/html/TREND/$time_id/progress.txt", $inc);
				
				# TODO: Fix by making a progress text file for every folder
				
				$list = querySDSS($objID, $time_id, $inc, $link, $log);
				$log->logInfo("returned size: ", count($list));
				$line_names = array();
				...
				while($line = mysqli_fetch_assoc($ret)){
					...
					$coords[] = [$line['RA'], $line['DECL'] ];
					
					$redshift[] = $line['Z'];
					
					//$log->logInfo("redshift: ", $line['Z'] );
				}
				
				/* free result set */
				mysqli_free_result($ret);
				
				#$log->logInfo("size after query: ", count($objid) );
				...
				#$log->logInfo("min rest: ", $min_rest);
				#$log->logInfo("max rest: ", $max_rest);
				
				#$log->logInfo("min wave: ", $min_obs);
				#$log->logInfo("max wave: ", $max_obs);
				...
				// 	$log->logInfo("rwave: ", $r);
				// }
				#$log->logInfo("before loop: ", count($list));
				
				for($i = 0; $i < count($list); $i+=500){
				
					/* check if there are still 100 to query for */
					...
					#$log->logInfo("query:", $query);
					
					$flux = mysqli_query($link,$query) or die("error at line 142: ".mysqli_error($link,$query));
					
					#$log->logInfo("wave rows:",mysql_num_rows($wave));
					#$log->logInfo("flux rows:",mysql_num_rows($flux));
				...
				#$log->logInfo("num lines: ", count($line_names));
				...
				//file_put_contents("/var/www/html/TREND/$time_id/progress.txt", 100);
