        ...
        create: function(opt, root) {
            ...
            $.each(opt.items, function(key, item){
                ...
                if (typeof item == "string") {
                    ...
                    // add icons
                    if (item.icon) {
                        //float: left; display: inline; position: relative; left: 0px; margin-left: -20px;
                        //$t.append($("<div/>"));
                        //$t.addClass("icon icon-" + item.icon);
                        var icon = $('<div></div>').prependTo($t);
                        icon.addClass("ui-icon ui-icon-" + item.icon);
                        icon.attr("style" , "float: left; display: inline; position: relative; left: 0px; margin-left: -20px;");
                    }
                }
        ...
        update: function(opt, root) {
            ...
            if (root === undefined) {
                ...
                opt.$menu.find('ul').andSelf().css({position: 'static', display: 'block'}).each(function(){
                    var $this = $(this);
                    $this.width($this.css('position', 'absolute').width() + 24)
                        .css('position', 'static');
