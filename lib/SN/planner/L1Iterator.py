...
class L1Iterator:
    def __init__(self, _targeting_list, _night_map, _night_num, _night_capacity, _strategy = 0):
    ...
    def get_capa_consume(self):
        ...
        for supnova in self.tlist:
            duration = supnova.get_obsDuration()
    ...
    def iterator(self):
        ...
        if self.nnum * self.ncapa * self.cutoff_ratio1 < self.get_capa_consume():
            pass
        else:
            # Distribution 1: full capacity all
            if self.strategy == 0:
                for i in range(self.nnum):
                    self.capa_distribution.append(self.ncapa)
            ...
            # Distribution 2: around evenly distributed capacity
                ...
                for supnova in self.tlist:
                        ...
                        if type(supnova.get_deadline(0, self.nmap)) is type(0.1):
                                base = (supnova.get_obsDuration() * supnova.get_obsTimes()) / self.nnum
                                for i in range(self.nnum):
                        ...
                        # case 3. first and second deadline both in range
                                ...
                                if supnova.get_obsTimes() > 3:
                                    base1 = (supnova.get_obsDuration() * 3) / (the_real_range1 + 1)
                                    base2 = (supnova.get_obsDuration() * (supnova.get_obsTimes() - 3)) / (the_real_range2 + 1)
                                else:
                                    base1 = (supnova.get_obsDuration() * supnova.get_obsTimes()) / (the_real_range1 + 1)
                                    base2 = 0
                ...
                for i in range(self.nnum):
                        ...
                        if new_capa > self.ncapa:
                                new_capa = self.ncapa
                        self.capa_distribution[i] = int(new_capa)
                
                self.capa_distribution[0] = self.ncapa
            ...
            # Distribution 3: reserve capacity and postpone current observation as much as possible
                ...
                for supnova in self.tlist:
                        ...
                        if type(supnova.get_deadline(0, self.nmap)) is type(0.1):
                                ...
                                for i in range(supnova.get_obsTimes()):
                                        self.capa_distribution[tmp_index] += supnova.get_obsDuration()
                                ...
                                if supnova.get_obsTimes() > 3:
                                    for i in range(3):
                                            self.capa_distribution[tmp_index1] += supnova.get_obsDuration()
                                    ...
                                    for i in range(supnova.get_obsTimes() - 3):
                                            self.capa_distribution[tmp_index2] += supnova.get_obsDuration()
                ...
                for i in range(self.nnum - 1):
                    index = self.nnum - i - 1
                    if self.capa_distribution[index] > self.ncapa:
                        more = self.capa_distribution[index] - self.ncapa
                        self.capa_distribution[index] = self.ncapa
