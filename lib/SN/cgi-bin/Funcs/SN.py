...
class SN:
    ...
    def get_remaining_work(self, _night_index, _night_map):

        duration = self.get_obsDuration(_night_index, _night_map)
        remaining = self.curr_status['remaining_instances'] * duration
        if remaining > 0:
                return remaining
    ...
    def get_obsDuration(self, _night_index, _night_map):
        import Funcs.ObsDuration
        
        curr_date = _night_map[_night_index]
        phase = (curr_date - self.Peakdate).days
        z = self.Redshift
        if z == 0:
            return 27
        duration = Funcs.ObsDuration.obsDuration(z, phase)
    ...
    def __str__(self):
        ...
        output += "Type: " + self.Type + "\tRedshift: " + str(self.Redshift) + "\tMag: " + str(self.Mag) + "\tBPeak: " + str(self.BPeak)
